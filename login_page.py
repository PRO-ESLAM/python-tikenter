...
class login_page:
    def __init__(self, root):
        ...
        self.frame1 = Frame(self.window, bg="#1D267D")
        self.frame1.place(x=0, y=0, width=450, relheight = 1)

        label1 = Label(self.frame1, text= "Billing", font=("times new roman", 40, "bold"), bg="#1D267D", fg="red").place(x=100,y=300)
        label2 = Label(self.frame1, text= "System", font=("times new roman", 40, "bold"), bg="#1D267D", fg="RoyalBlue1").place(x=162,y=300)
        ...
        self.create_button = Button(self.frame3,text="Create New Account",command=self.redirect_window,font=("times new roman",18, "bold"),bd=0,cursor="hand2",bg="#0C134F",fg="white").place(x=80,y=320,width=250)
    ...
    def login_func(self):
        if self.email_entry.get()=="" or self.password_entry.get()=="":
            ...
            try:
                ...
                cur = con.cursor()
                cur.execute("select * from regester where email=%s and password=%s",(self.email_entry.get(),self.password_entry.get()))
                row=cur.fetchone()
                if row == None:
                    messagebox.showerror("Error!","Invalid USERNAME & PASSWORD",parent=self.window)
                else:
                    myvar=messagebox.askyesno("Bill system ","Acess only Admin")
                    if myvar > 0 :
                        self.new_window=Toplevel(self.window)
                        self.app=BillApp(self.new_window)
                    else:
                        messagebox.showinfo("Erorr","Wrong Open ",parent=self.window)
                        self.reset_fields()
